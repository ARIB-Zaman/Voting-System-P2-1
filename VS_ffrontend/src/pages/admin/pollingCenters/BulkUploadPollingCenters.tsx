...
import { UploadCloud, FileText, Download, Building2, AlertTriangle, CheckCircle2 } from 'lucide-react';
import {
  Table,
  TableBody,
  TableCell,
...
const EXPECTED_HEADERS = ['name', 'address', 'constituency_id', 'latitude', 'longitude'];

const BulkUploadPollingCenters: React.FC = () => {
  ...
  const downloadSampleCSV = () => {
    const csvContent = "data:text/csv;charset=utf-8," 
      ...
      + "Mirpur-10 Center,House 123,10,23.8103,90.4125\n"
      + "Dhanmondi-5 Center,House 456,12,23.7465,90.3740";
    
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", "sample_polling_centers.csv");
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await fetch(`${API}/admin-polling-centers/bulk`, {
        ...
        body: JSON.stringify({ centers: previewData }),
      });
      ...
      if (data.results.failed === 0) {
        toast.success(`Successfully registered all ${data.results.inserted} polling centers!`);
        setFile(null);
        setPreviewData([]);
      } else {
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-black tracking-tight">Bulk Upload Centers</h1>
          <p className="text-sm text-muted-foreground">Import multiple polling centers using a CSV file</p>
        </div>
      </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-muted/50">
                      ...
                      <TableHead className="text-xs font-bold uppercase px-4 py-3">Address</TableHead>
                      <TableHead className="text-xs font-bold uppercase px-4 py-3">C. ID</TableHead>
                  ...
                  <TableBody>
                    {previewData.slice(0, 100).map((row, i) => (
                      <TableRow key={i} className="text-sm">
                        ...
                        <TableCell className="px-4 py-2 text-muted-foreground truncate max-w-[200px]" title={row.address}>
                          {row.address || <span className="text-destructive text-xs italic">Missing</span>}
                        </TableCell>
                        <TableCell className="px-4 py-2 font-mono text-xs">
                          {row.constituency_id || <span className="text-destructive italic">Missing</span>}
                        </TableCell>
...
export default BulkUploadPollingCenters;
