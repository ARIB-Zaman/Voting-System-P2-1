...
const API = 'http://localhost:3001/api/ro';
...
const RoConstituencyDetails: React.FC = () => {
    ...
    }>({ name: '', address: '', status: 'OPEN', presiding_officer_name: '' });
    ...
    const fetchInfo = useCallback(async () => {
        if (!cId) return;
        const res = await fetch(`${API}/constituency/${cId}`, { credentials: 'include' });
    ...
    const fetchCenters = useCallback(async () => {
        if (!cId) return;
        const res = await fetch(`${API}/constituency/${cId}/polling-centers`, { credentials: 'include' });
    ...
    useEffect(() => {
        (async () => {
            setLoading(true);
            try {
                const [infoData, centersData] = await Promise.all([fetchInfo(), fetchCenters()]);
                if (infoData) {
                    setInfo(infoData);
                    // Set default status for the add row based on election date range
    ...
    const addCenter = async () => {
        ...
        setAdding(true);
        try {
            const res = await fetch(`${API}/constituency/${cId}/polling-centers`, {
                method: 'POST',
                credentials: 'include',
                ...
                body: JSON.stringify({
                    ...
                    status: newStatus,
                    presiding_officer_name: newPOName.trim() || undefined,
                }),
            });
            if (!res.ok) {
    ...
    const startEdit = (pc: PollingCenter) => {
        ...
        setEditForm({
            ...
            address: pc.address,
            status: pc.status,
            presiding_officer_name: pc.presiding_officer_name ?? '',
        });
    };
    ...
    const saveEdit = async () => {
        ...
        setSaving(true);
        try {
            const res = await fetch(`${API}/polling-centers/${editingId}`, {
                method: 'PUT',
                credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    ...
                    presiding_officer_name: editForm.presiding_officer_name,
                }),
            });
            if (!res.ok) {
                const err = await res.json();
    ...
    const deleteCenter = async (centerId: number) => {
        ...
        try {
            const res = await fetch(`${API}/polling-centers/${centerId}`, {
                method: 'DELETE',
                credentials: 'include',
            });
                        ...
                        {showAddRow && (
                            <TableRow className="bg-primary/5">
                                ...
                                <TableCell className="px-6 py-3">
                                    <Select
                                        value={newStatus}
                                        onValueChange={(v) => setNewStatus(v as PCStatus)}
                                    >
                                        ...
                                        <TableRow key={pc.center_id} className="bg-primary/5">
                                            ...
                                            <TableCell className="px-6 py-3">
                                                <Select
                                                    ...
                                                    onValueChange={(v) =>
                                                        setEditForm((f) => ({ ...f, status: v as PCStatus }))
                                                    }
                                                >
                                                    <SelectTrigger className="h-9">
                                                        <SelectValue />
                                            ...
                                            <TableCell className="px-6 py-3">
                                                <Input
                                                    value={editForm.presiding_officer_name}
                                                    onChange={(e) =>
                                                        setEditForm((f) => ({
                                                            ...f,
                                                            presiding_officer_name: e.target.value,
                                                        }))
                                                    }
                                                    placeholder="Officer name (optional)"
                                                    className="h-9"
                                        ...
                                        <TableCell className="px-6 py-3 text-right">
                                            <div className="flex gap-1.5 justify-end">
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() =>
                                                        navigate(
                                                            `/homeRO/constituency/${cId}/polling-center/${pc.center_id}`
                                                        )
                                                    }
                                                ...
                                                <AlertDialog>
                                                    ...
                                                    <AlertDialogContent>
                                                        <AlertDialogHeader>
                                                            ...
                                                            <AlertDialogDescription>
                                                                This will permanently remove this polling center.
